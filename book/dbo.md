...
-   Предложения Языка Определения Данных - Data Definition Language (DDL). DDL (в ANSI он называется Языком Описания Схем – Shema Definition Language) состоит из тех команд, которые создают объекты (таблицы, представления, индексы) в БД. Также DDL предложения используются для определения
